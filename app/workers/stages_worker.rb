    ...
    puts b
  	if b["mandatory"] == "true"
  		if b["name"] == "Call Customer"
  			...
  			a.user.tasks.create(title: "Schedule demo or other stages for #{a.full_name} ", content: "Please call {a.full_name} to schedule demo on #{a.phone}")
 			...
 			Customer.create(full_name: "#{a.full_name}", email: "#{a.email}", phone: "#{a.phone}", location: "#{a.location}", managed_by: "#{a.managed_by}")
